...
namespace NaturalSelectionLib
{
    ...
    public class NaturalSelectionLib : BaseUnityPlugin
    {
        ...
        public static string DebugStringHead(EnemyAI? __instance)
        {
            ...
            else return __instance?.name + " ID: " + __instance?.NetworkObjectId + ": ";
        }
        ...
        {            
            ...
            for (int i = 0; i < importEnemyList.Count; i++)
            {
                ...
                if (importClosestEnemy.isEnemyDead)
                {
                    if (!includeTheDead)
                    {
                        if (debugLibrary && debugSpam) LibraryLogger.LogError(DebugStringHead(__instance) + ", " + DebugStringHead(__instance) + " is dead! Assigning new tempClosestEnemy from importEnemyList...");
                ...
                {
                    if (debugLibrary && debugSpam) LibraryLogger.LogWarning(DebugStringHead(__instance) + importEnemyList[i] + ", ID: " + importEnemyList[i].GetInstanceID() + " is already assigned as closestEnemy");
                    continue;
                }
                ...
                if (Vector3.Distance(__instance.transform.position, importEnemyList[i].transform.position) < Vector3.Distance(__instance.transform.position, importClosestEnemy.transform.position))
                {
                    ...
                    if (debugLibrary) LibraryLogger.LogInfo(DebugStringHead(__instance) + "Assigned " + importEnemyList[i] + ", ID: " + importEnemyList[i].GetInstanceID() + " as new closestEnemy. Distance: " + Vector3.Distance(__instance.transform.position, importClosestEnemy.transform.position));
